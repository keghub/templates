...
namespace EMG.WcfWindowsServiceWithNybus.Installers
{
    public class NybusInstaller : IWindsorInstaller
    {
        public void Install(IWindsorContainer container, IConfigurationStore store)
        {
            ...
            container.Register(Component.For<NybusOptions>().Forward<INybusOptions>().OnCreate(o =>
            {
                o.UseCastleWindsor(container.Kernel);
            }));

            container.Register(Component.For<MassTransitConnectionDescriptor>().UsingFactoryMethod(kernel =>
            {
