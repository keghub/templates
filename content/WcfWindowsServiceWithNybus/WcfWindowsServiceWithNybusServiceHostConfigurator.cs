...
namespace EMG.WcfWindowsServiceWithNybus
{
    ...
    */
    public class WcfWindowsServiceWithNybusServiceHostConfigurator : IServiceHostConfigurator<WcfWindowsServiceWithNybus>
    {
        private readonly WcfHostingOptions _options;

        public WcfWindowsServiceWithNybusServiceHostConfigurator(WcfHostingOptions options)
        {
            _options = options ?? throw new ArgumentNullException(nameof(options));
        }

        public void Configure(IServiceHost<WcfWindowsServiceWithNybus> host)
        {
            /*
            ...
            */
            ...
            //host.AddBasicHttpEndpoint(typeof(IWcfWindowsServiceWithNybus), 9999, hostname: _options.AnnouncedHost, configureBinding: binding => binding.UseDefaults());

            /* Adds a WSHttp endpoint on port 9999 with suffix: "ws". Uses default configuration. */
            //host.AddWSHttpEndpoint(typeof(IWcfWindowsServiceWithNybus), 9999, hostname: _options.AnnouncedHost, configureBinding: binding => binding.UseDefaults());

            /* Adds a WebHttp endpoint on port 9999 with suffix: "web". Uses default configuration. */
            //host.AddWebHttpEndpoint(typeof(IWcfWindowsServiceWithNybus), 9999, hostname: _options.AnnouncedHost, configureBinding: binding => binding.UseDefaults());

            /* Adds a NamedPipe endpoint with suffix: "echo". Uses default configuration. */
            //host.AddNamedPipeEndpoint(typeof(IWcfWindowsServiceWithNybus), "echo", configureBinding: binding => binding.UseDefaults());

            /* Adds a NetTcp endpoint on port 9998. Uses default configuration. */
            host.AddNetTcpEndpoint(typeof(IWcfWindowsServiceWithNybus), 9998, hostname: _options.AnnouncedHostName, configureBinding: binding => binding.UseDefaults());
